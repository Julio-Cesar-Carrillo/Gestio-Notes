<?php
    ...
    $sql = "SELECT N.*, A.nombre as 'nombre', A.apellido1 as '1r apellido', A.apellido2 as '2o apellido' , M.nombre as 'asignatura' FROM tbl_notas N  INNER JOIN tbl_alumnos A ON N.id_alumno = A.id  INNER JOIN tbl_asignaturas M ON N.id_asignatura=M.id WHERE N.id_alumno=?;";
    ...
    {
        echo "<p>Aún falta añadir 1 nota.<p>
        <p><button><a href='./crear_nota.php?id={$alumno_id}&id_asignatura={$id_asignatura}'>Añadir nota ahora</a></button></p>";
    }
?>
        ...
        <table border=1>
            ...
            <?php
                foreach ($nota as $alumno) 
                {
                    ...
                    echo "<tr>
                    <td> {$alumno["nombre"]} {$alumno["1r apellido"]} {$alumno["2o apellido"]} </td>  
